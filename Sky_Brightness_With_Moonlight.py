...
class SkyBrightness:
    def __init__(self,ra,dec,mjd,date,filtre,airmass_opsim,extinction=0.172, skyBrightness=21.587):
        ...
        #print 'airmass here',x(self.moonZD_DEG),xa(self.moonZD_DEG),x(targetZD_DEG),xa(targetZD_DEG),self.distance2moon_RAD,self.moonRA_RAD, self.moonDec_RAD,lha_RAD,zd_RAD,alt_RAD,self.lst_RAD,targetZD_DEG,self.lst_RAD 
    ...
    def Get_Sidereal_Time_at_Site(self):
        ...
        mjd = (float(self.date) / float(DAY)) + float(self.simEpoch)
        print 'hello mjd',mjd
        lst_RAD = pal.gmst(mjd) + self.lon_RAD
        if lst_RAD < 0:
            lst_RAD += TWOPI
